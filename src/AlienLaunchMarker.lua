...
function AlienLaunchMarker:init(world)
    ...
    -- whether the arrow is showing the aim
    self.aiming = false 

    -- whether the alien is launched and stop rendering the preview
    self.launched = false

    -- the alien will eventually spawn
    self.alien = nil 
end 

function AlienLaunchMarker:update(dt)
    
    -- perform everything here as long as launch not done
    if not self.launched then

        -- grab mouse coordinates
        local x, y = push:toGame(love.mouse.getPosition())
        
        -- if mouse is clicked and  alien not launched, show arrow preview
        if love.mouse.wasPressed(1) and not self.launched then
            self.aiming = true

        -- if mouse released, launch an Alien
